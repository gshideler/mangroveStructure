...
pcqm.method <- function(x, 
  ...
  if("height" %in% colnames(x)) species_height <- plyr::ddply(x, "Species", summarize, n = length(SamplingPoint), Mean = round(mean(height), digits=2), SD = round(sd(height), digits=2), Min = min(height), Max = max(height))
