...
pcqm.indices <- function(x, 
  ...
  if(sizebin==T){
    ...
    plot(x$dbh, x$height, pch=21, xlab="Tree DBH", ylab="Tree Height", cex.lab = 1.2, cex.axis = 1.1, col="black", bg = c("burlywood3", "burlywood4", "saddlebrown")[x$sizefac], ylim=c(0,max(x$height)+2), xlim=c(0,max(x$dbh)+2))
